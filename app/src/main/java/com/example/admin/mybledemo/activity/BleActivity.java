...
import com.example.admin.mybledemo.utils.HexUtil;
 ...
 */
@ContentView(R.layout.activity_ble) public class BleActivity extends BaseActivity {
    private String TAG = BleActivity.class.getSimpleName();
    @ViewInit(R.id.listView) ListView mListView;
    private LeDeviceListAdapter mLeDeviceListAdapter;
    private Ble<BleDevice> mBle;
    private String path = Environment.getExternalStorageDirectory() + "/aceDownload/";
    ...
    private void initBle() {
        UUID[] uuids = new UUID[] {
                ...
                UUID.fromString("e093f3b5-00a3-a9e5-9eca-40016e0edc24")
        };
        mBle = Ble.options().setLogBleExceptions(true)//设置是否输出打印蓝牙日志
                .setThrowBleException(true)//设置是否抛出蓝牙异常
                .setAutoConnect(true)//设置是否自动连接
                .setFilterScan(false)//设置是否过滤扫描到的设备
                .setConnectFailedRetryCount(3).setConnectTimeout(10 * 1000)//设置连接超时时长
                .setScanPeriod(12 * 1000)//设置扫描时长
                .setUuidService(UUID.fromString("e093f3b5-00a3-a9e5-9eca-40016e0edc24"))//设置主服务的uuid
                .setUuidWriteCha(
                        UUID.fromString("e093f3b5-00a3-a9e5-9eca-40036e0edc24"))//设置可写特征的uuid
                .setUuidServicesExtra(uuids).create(getApplicationContext());
        //3、检查蓝牙是否支持及打开
    ...
    @CheckConnect //检查是否连接
    @OnClick({
            R.id.test, R.id.readRssi, R.id.sendData, R.id.updateOta, R.id.requestMtu,
            R.id.sendEntityData, R.id.cancelEntity
    })
    ...
    private void sendEntityData() throws IOException {
        ...
        mBle.writeEntity(mBle.getConnetedDevices().get(0), data, 20, 50,
                new BleWriteEntityCallback<BleDevice>() {
                    @Override
    ...
    private void requestMtu() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            mBle.setMTU(mBle.getConnetedDevices().get(0).getBleAddress(), 96,
    ...
    private void updateOta() {
        ...
        boolean result =
                mOtaManager.startOtaUpdate(file, (BleDevice) mBle.getConnetedDevices().get(0),
                        mBle);
        L.e("OTA升级结果:", result + "");
    }
    ...
    private void sendData() {
        String str =
                "FEFB530000D62099CEC1EBDB292497D6618991E91320EC6BFAE019FD0B0440BFA4A40F916AA5EA3CF971CB3487959B8ED3E97D96E8EEDD340DF88EB04BB8C2AFE830244E8C85DA7B3C3D61BB45826242DC676DC155C42D";
        List<BleDevice> list = Ble.getInstance().getConnetedDevices();
        synchronized (mBle.getLocker()) {
            for (BleDevice device : list) {
                boolean ddd = mBle.write(device, HexUtil.fromHexString(str),
                ...
                Log.e("ddddddddd", "ddddddddd" + ddd);
                ToastUtil.showToast("ddddddddd" + ddd);
            }
