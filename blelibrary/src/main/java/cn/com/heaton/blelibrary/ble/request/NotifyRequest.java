 ...
 */
@Implement(NotifyRequest.class) public class NotifyRequest<T extends BleDevice>
        implements NotifyWrapperLisenter<T> {
        ...
        TaskExecutor.mainThread(new Runnable() {
            ...
            public void run() {
                for (BleNotiftCallback callback : mNotifyCallbacks) {
                    callback.onChanged(device, characteristic, data);
